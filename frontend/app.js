...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateKeyStatusUI(hasKey) {
    if (hasKey) {
      ...
      modalKeyStatus.textContent = "Configured in .env";
      ...
      keyStatusText.textContent = "No .env Key (Heuristic Mode)";
    ...
    jobs.forEach((job) => {
      ...
      card.innerHTML = `
        ...
        <div class="flex items-center justify-between border-t border-slate-800/80 pt-3">
          <span class="text-[11px] text-slate-500">${job.posted_date || "Active UK vacancy"}</span>
          <a href="${job.url}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-500 shadow-md shadow-indigo-600/25 transition">
